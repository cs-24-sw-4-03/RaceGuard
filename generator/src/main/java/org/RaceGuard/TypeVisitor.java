...
import org.RaceGuard.Exceptions.*;
...
public class TypeVisitor implements NodeVisitor {
    ...
    private String arithTypeConvert(String type1, String type2){
        ...
        if (type1.equals(RaceGuardE.INT.getValue()) && type2.equals(RaceGuardE.DOUBLE.getValue())){
            return RaceGuardE.DOUBLE.getValue(); //if types are int and double, return double
        }
        if (type1.equals(RaceGuardE.DOUBLE.getValue()) && type2.equals(RaceGuardE.INT.getValue())){
            return RaceGuardE.DOUBLE.getValue(); //if types are double and int, return double
        }
        return null;
    }
    ...
    private boolean canConvert(String assignTo, String assignFrom){
        ...
        if (assignTo.equals(RaceGuardE.DOUBLE.getValue()) && assignFrom.equals(RaceGuardE.INT.getValue())){
            return true; //if types are int and double, return true
        }
        if (assignTo.equals(RaceGuardE.DOUBLE_ARRAY.getValue()) && assignFrom.equals(RaceGuardE.INT_ARRAY.getValue())){
            return true; //if types are int[] and double[], return true
        }
        if (assignTo.equals(RaceGuardE.DOUBLE_ARRAY_2D.getValue()) && assignFrom.equals(RaceGuardE.INT_ARRAY_2D.getValue())){
            return true; //if types are int[][] and double[][], return true
        }
    ...
    public void visit(ReturnStatementNode node) {
        ...
        if(!parent.getType().equals(RaceGuardE.VOID.getValue())) {
            ...
            if(node.getChildren().isEmpty()){ //since the the return type is void, the return statement should be empty
                node.setType(RaceGuardE.VOID.getValue());
            }else {
    ...
    public void visit(LocalMethodBodyNode node) {
        this.visitChildren(node);
        try{
        if(!node.getParent().getType().equals(RaceGuardE.VOID.getValue())){ //If the method does not return void
            ...
            node.setType(RaceGuardE.VOID.getValue());//If there is a return statement, it is checked in visit(ReturnStatement node) that it is "return;". So an error should already have been produced
        }
        }
    ...
    public void visit(BoolCompareNode node) {
        ...
        try {
            for (AstNode child : node.getChildren()) {
                if (!child.getType().equals(RaceGuardE.BOOL.getValue())) { //If the children are not of type bool, throw an exception
            ...
            node.setType(RaceGuardE.BOOL.getValue());
        }
    ...
    public void visit(MethodDclNode node) {
        try {
            ...
            this.visitChildren(node);
            if (node.getMethodType().equals(RaceGuardE.LOCAL.getValue())){ //If the method type is local
            ...
            else if (node.getMethodType().equals(RaceGuardE.ON.getValue())) { //If the method type is on
    ...
    public void visit(IntegerNode node) {
        try {
            ...
            node.setType(RaceGuardE.INT.getValue());
        }
    ...
    public void visit(DoubleNode node) {
        try {
            ...
            node.setType(RaceGuardE.DOUBLE.getValue());
        }
    ...
    public void visit(StringNode node) {
        try {
            ...
            node.setType(RaceGuardE.STRING.getValue());
        }
    ...
    public void visit(BoolAndExpNode node) {
        ...
        try {
            for (AstNode child : node.getChildren()) {
                if (!child.getType().equals(RaceGuardE.BOOL.getValue())) {
    ...
    public void visit(NegatedBoolNode node) {
        this.visitChildren(node);
        try {
            if (node.getChildren().get(0).getType().equals(RaceGuardE.BOOL.getValue())) { //If the child is of type bool
                node.setType(RaceGuardE.BOOL.getValue());
            } else {
    ...
    private boolean compareExpTypeMatching(String operator, String leftType, String rightType){
        if (leftType.equals(rightType) && leftType.equals(RaceGuardE.INT.getValue()) || leftType.equals(RaceGuardE.DOUBLE.getValue())){
            return true; //If the types are the same and are either int or double
        }
        else if (operator.equals("==") && leftType.equals(rightType) && leftType.equals(RaceGuardE.BOOL.getValue())){
    ...
    public void visit(PrintCallNode node) {
        ...
        try {
            for (AstNode child : node.getChildren()) {
                if (!child.getType().equals(RaceGuardE.STRING.getValue())) { //If the child is not of type string
                    if (child.getType().equals(RaceGuardE.INT.getValue()) || child.getType().equals(RaceGuardE.DOUBLE.getValue()) || child.getType().contains("[]")) { //If the child is of type int or double we cn convert it to string
                        continue;
                    }
